...
//        for (h, page) in enumerate(keyboard.pages) {  // Commented by guoc for speed up load
        for (h, page) in enumerate([keyboard.pages[0]]) {   // Added by guoc for speed up load
            let numRows = page.rows.count
    ...
    /* Added by guoc for speed up load */
    
    func lazyCreateViews(keyboard: Keyboard) {
        let specialKeyVibrancy: VibrancyType? = (self.darkMode ? VibrancyType.DarkSpecial : VibrancyType.LightSpecial)
        let normalKeyVibrancy: VibrancyType? = (self.darkMode ? VibrancyType.DarkRegular : nil)
        
        for (h, page) in enumerate([keyboard.pages[1], keyboard.pages[2]]) {
            let numRows = page.rows.count
...
//        for page in model.pages { // Commented by guoc for speed up load
        for page in [model.pages[0]] {   // Added by guoc for speed up load
            let numRows = page.rows.count
    ...
    func lazyLayoutKeys(model: Keyboard, views: [Key:KeyboardKey], bounds: CGRect) {
        if bounds.height == 0 || bounds.width == 0 {
            return
        }
        ...
        //        for page in model.pages { // Commented by guoc for speed up load
        for page in [model.pages[1], model.pages[2]] {   // Added by guoc for speed up load
            let numRows = page.rows.count
