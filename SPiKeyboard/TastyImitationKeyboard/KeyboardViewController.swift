...
//        for page in keyboard.pages {  // Commented by guoc for speed up
        for page in [keyboard.pages[0]] {    // Added by guoc for speed up
    ...
    func lazySetupKeys() {
        ...
        for page in [keyboard.pages[1], keyboard.pages[2]] {
            for rowKeys in page.rows { // TODO: quick hack
                for key in rowKeys {
                    var keyView = self.layout!.viewForKey(key)! // TODO: check
...
//                (self.textDocumentProxy as? UITextDocumentProxy)?.insertText(" ")    // Commented by guoc for changing to Chinese period
    ...
    var hasLazyLoad = false    // Added by guoc for speed up
    func setMode(mode: Int) {
        /* Added by guoc for speed up */
        if mode != 0 && hasLazyLoad == false {
            hasLazyLoad = true
            self.keyboard = lazyDefaultKeyboard(withPreviousKeyboard: self.keyboard)
            self.layout?.lazyCreateViews(self.keyboard)
            self.lazySetupKeys()
            self.layout?.lazyLayoutKeys(self.keyboard, views: self.layout!.modelToView, bounds: self.layout!.superview.bounds)
        }
        /* Added end */
